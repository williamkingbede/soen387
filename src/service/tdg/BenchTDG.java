...
public class BenchTDG {
	...
	public static int insert(long gameId, long playerId, long version) throws SQLException{
		...
		String query = "INSERT INTO bench (gameid,playerid,version) VALUES (?,?,?);";
	...
	public static int insert(long gameId, long playerId, long cardid, long version) throws SQLException{
		Connection con = ConnectionManager.getConnection();
		String query = "INSERT INTO bench (gameid,playerid,cardid,version) VALUES (?,?,?,?);";
		PreparedStatement ps = con.prepareStatement(query, PreparedStatement.RETURN_GENERATED_KEYS);
		ps.setLong(1, gameId);
		ps.setLong(2, playerId);
		ps.setLong(3, cardid);
	...
	public static int update(long gameId, long playerId, long cardid, long version) throws SQLException{
		Connection con = ConnectionManager.getConnection();
		String query = "UPDATE bench SET version=version+1, cardid = ? where gameid = ? AND playerid = ? AND version=?;";
	...
	public static int update(long gameId, long playerId, long version) throws SQLException{
		Connection con = ConnectionManager.getConnection();
		String query = "UPDATE bench SET version=version+1 where gameid = ? AND playerid = ? AND version=?;";
		PreparedStatement ps = con.prepareStatement(query);
		ps.setLong(1, gameId);
		ps.setLong(2, playerId);
		ps.setLong(3, version);
	...
	public static int delete(long gameId, long playerId, long version) throws SQLException{
		Connection con = ConnectionManager.getConnection();
		String query = "DELETE FROM bench WHERE gameid=? AND playerid= ? AND version = ?;";
		PreparedStatement ps = con.prepareStatement(query);
		ps.setLong(1, gameId);
		ps.setLong(2, playerId);
		ps.setLong(3, version);
